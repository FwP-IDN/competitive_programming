...
int addd(int bit, int idx, int val) {
	return bit + val * p5[idx];
}
...
int solve(int idx, int pos, int bit) {
	...
	if(idx < N && sum(bit) - gett(bit, a[idx]) + 1 <= 4) {
		ret = min(ret, abs(a[idx] - pos) + gett(bit, a[idx]) + 1 + solve(idx + 1,a[idx], addd(set0(bit, a[idx]), b[idx], 1)));
	}
